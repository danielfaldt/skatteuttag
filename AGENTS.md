...
## 9. Token-efficient workflow

If `rtk` is available, prefer it for noisy shell commands such as:

- `rtk git status`
- `rtk git diff`
- `rtk git log --oneline -20`
- `rtk docker compose ps`
- `rtk docker compose logs --tail=200`
- `rtk pytest -q`

If `rtk` output is insufficient, rerun only the specific command with raw output.
...
3. `README.md`
4. `TODO.md`
5. Older docs or archived notes

## Task tracking

GitHub Issues is the source of truth for open and planned work.
See https://github.com/danielfaldt/taxsplit/issues
`TODO.md` is deprecated — kept for historical reference only. Do not add new tasks there.
